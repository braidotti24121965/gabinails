...
import { appointments as initialAppointments, clients as demoClients, inventory, money, professionals as demoProfessionals, recovery, services, type Appointment, type AppointmentStatus } from "@/lib/demo-data";
...
function Dashboard({ go }: { go: (v: View) => void }) {
  return <main className="page-content space-y-7"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric onClick={() => go("agenda")} label="Agendamentos hoje" value="16" detail="13 confirmados · 2 pendentes" icon={CalendarDays} /><Metric onClick={() => go("finance")} label="Faturamento do dia" value="R$ 1.487" detail="+12% comparado ao último domingo" icon={TrendingUp} /><Metric onClick={() => go("finance")} label="Recebido hoje" value="R$ 1.217" detail="R$ 270 ainda pendentes" icon={Wallet} /><Metric onClick={() => go("agenda")} label="Ocupação" value="84%" detail="6 horários ainda disponíveis" icon={Clock3} /></div><div className="grid gap-5 xl:grid-cols-[1.45fr_.8fr]"><section className="card"><SectionTitle title="Agenda de hoje" subtitle="Próximos horários e andamento" action={<button onClick={() => go("agenda")} className="btn-outline">Ver agenda <ArrowRight size={15} /></button>} /><div className="space-y-2">{initialAppointments.slice(1, 5).map((a, i) => <button onClick={() => go(a.status === "Em atendimento" ? "attendance" : "agenda")} key={a.id} className="flex w-full items-center gap-3 rounded-md border border-[#E7EDF3] p-3 text-left transition hover:border-primary/40 hover:bg-bg"><div className="w-12 shrink-0 text-center"><p className="text-sm font-semibold">{a.time}</p><p className="text-[10px] text-muted">{a.end}</p></div><div className={`h-10 w-1 rounded-full ${i === 0 ? "bg-primary" : i === 3 ? "bg-warning" : "bg-navy/25"}`} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{a.client}</p><p className="truncate text-xs text-muted">{a.service} · {a.professional}</p></div><Badge tone={statusTone(a.status)}>{a.status}</Badge></button>)}</div></section><section className="card"><SectionTitle title="Próxima cliente" subtitle="Em 12 minutos" /><div className="rounded-md bg-primary-light p-4"><button onClick={() => go("clients")} className="flex w-full items-center gap-3 text-left"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-primary text-sm font-semibold text-white">CM</div><div><p className="font-semibold">Clara Martins</p><p className="text-xs text-muted">10:45 · Esmaltação em gel</p></div></button><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={() => go("attendance")} className="btn-primary"><Check size={15} /> Chegou</button><button onClick={() => go("automations")} className="btn-outline"><MessageCircle size={15} /> WhatsApp</button></div></div><div className="mt-4 grid grid-cols-2 gap-3"><SmallMetricLink go={go} label="Faturamento mensal" value="R$ 31.840" target="finance" /><SmallMetricLink go={go} label="Ticket médio" value="R$ 128" target="finance" /><SmallMetricLink go={go} label="Clientes novas" value="18" target="clients" /><SmallMetricLink go={go} label="Taxa de retorno" value="71%" target="clients" /></div></section></div><section className="card"><SectionTitle title="Clientes para recuperar" subtitle="Oportunidades priorizadas por manutenção e tempo sem atendimento" /><div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Cliente</th><th>Motivo</th><th>Situação</th><th>Receita potencial</th><th className="text-right">Ações</th></tr></thead><tbody>{recovery.map(r => <tr key={r.client}><td><button onClick={() => go("clients")} className="font-medium hover:text-primary">{r.client}</button></td><td><Badge tone={r.tone}>{r.reason}</Badge></td><td className="text-muted">{r.delay}</td><td className="font-medium">{money.format(r.value)}</td><td><div className="flex justify-end gap-2"><button onClick={() => go("automations")} className="btn-ghost !min-h-8 !px-3">Preparar mensagem</button><button onClick={() => go("agenda")} className="btn-outline !min-h-8 !px-3">Agendar</button></div></td></tr>)}</tbody></table></div></section></main>;
}

function Agenda({ rows, onNew, onAttendance, onAction, onCancel }: { rows: Appointment[]; onNew: () => void; onAttendance: () => void; onAction: (mode: "view" | "edit", index: number) => void; onCancel: (index: number) => void }) {
  return <main className="page-content"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div className="flex rounded-md border border-[#DBE3EC] bg-white p-1"><button className="rounded-sm bg-primary px-4 py-2 text-xs font-medium text-white">Dia</button><button className="px-4 py-2 text-xs text-muted">Semana</button><button className="px-4 py-2 text-xs text-muted">Mês</button></div><button onClick={onNew} className="btn-primary"><Plus size={16} />Novo agendamento</button></div><div className="mb-4 grid gap-3 sm:grid-cols-3"><select className="field-input" aria-label="Profissional"><option>Todas as profissionais</option>{demoProfessionals.map(p => <option key={p.name}>{p.name}</option>)}</select><select className="field-input" aria-label="Serviço"><option>Todos os serviços</option>{services.map(s => <option key={s.name}>{s.name}</option>)}</select><select className="field-input" aria-label="Status"><option>Todos os status</option><option>Confirmado</option><option>Em atendimento</option><option>Aguardando sinal</option></select></div><section className="card !p-0 overflow-hidden"><div className="border-b border-[#E7EDF3] bg-[#F7F9FC] px-5 py-3 text-xs text-muted">08:00 — 19:00 · Intervalos de 15 minutos</div><div className="divide-y divide-[#E7EDF3]">{rows.map((a, index) => <div key={a.id} className="flex w-full items-center gap-3 px-4 py-3 hover:bg-bg sm:px-5"><button onClick={a.status === "Em atendimento" ? onAttendance : () => onAction("view", index)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><div className="w-14 shrink-0"><p className="font-semibold">{a.time}</p><p className="text-[10px] text-muted">{a.end}</p></div><div className="h-12 w-1 rounded-full bg-primary" /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate font-medium">{a.client}</p>{a.source === "Online" && <Badge tone="blue">Online</Badge>}</div><p className="truncate text-xs text-muted">{a.service} · {a.professional}</p></div><div className="hidden text-right md:block"><p className="font-medium">{money.format(a.price)}</p><p className="text-[10px] text-muted">{a.paid ? `${money.format(a.paid)} recebido` : "Pagamento pendente"}</p></div><Badge tone={statusTone(a.status)}>{a.status}</Badge></button><RowActions onView={() => onAction("view", index)} onEdit={() => onAction("edit", index)} onDelete={() => onCancel(index)} deleteLabel="Cancelar" /></div>)}</div></section></main>;
}
...
export function NailStudioApp({ initialClients = demoClients, initialProfessionals = demoProfessionals }: { initialClients?: ClientItem[]; initialProfessionals?: ProfessionalItem[] }) {
  ...
  const content = (() => {
    ...
    if (view === "agenda") return <Agenda rows={rows} onNew={() => setBooking(true)} onAttendance={() => setView("attendance")} onAction={(mode, index) => openEntity("appointment", mode, index)} onCancel={index => confirmAction("Cancelar este agendamento? O histórico será preservado.", () => { setRows(current => current.map((item, i) => i === index ? { ...item, status: "Cancelado" } : item)); notify("Agendamento cancelado e horário liberado."); })} />;
